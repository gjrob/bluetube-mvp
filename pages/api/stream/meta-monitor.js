...
// components/FounderStreamOverlay.js
import { useEffect, useState } from 'react';

export default function FounderStreamOverlay({ streamId }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`founder-stream-${streamId}`)
      .on('postgres_changes', 
      ...
      .on('postgres_changes',
        { event: '*', schema: 'public', table: 'self_healing_logs' },
        (payload) => {
          // Show AI fixes in real-time
          setMetrics(prev => ({
            ...prev,
            last_fix: payload.new
          }));
        }
      )
      .subscribe();
  ...
  return (
    <div className="absolute top-4 right-4 bg-black/80 text-white p-4 rounded-lg">
      <div className="text-xs opacity-60 mb-2">PLATFORM STATUS</div>
      
      {/* Revenue Counter */}
      <div className="text-2xl font-bold text-green-400">
        ${metrics.revenue_today.toFixed(2)}
      </div>
      <div className="text-xs opacity-60">Today's Revenue</div>
      
      {/* AI Status */}
      <div className="mt-3 flex items-center gap-2">
        <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
        <div className="text-xs">AI: {metrics.ai_status}</div>
      </div>
      
      {/* Last AI Fix */}
      {metrics.last_fix && (
        <div className="mt-2 text-xs bg-blue-500/20 p-2 rounded">
          🤖 Fixed: {metrics.last_fix.issue}
        </div>
      )}
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
        <div>
          <div className="opacity-60">Viewers</div>
          <div className="font-bold">{metrics.live_viewers}</div>
        </div>
        <div>
